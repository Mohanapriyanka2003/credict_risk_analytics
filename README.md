...
Python VersionFastAPIScikit-Learn
License
...
PD (Probability of Default): Logistic Regression classifier modeling default probability (AUC=0.792).
LGD (Loss Given Default): Random Forest regressor modeling loss percentage based on property collateral coverage (R 2 =0.994).
EAD (Exposure at Default): Random Forest regressor modeling outstanding balance at default (R 2 =0.997).
...
PD (Probability of Default): Continuous probability (0.0 to 1.0) that a borrower defaults within a 12-month horizon.
LGD (Loss Given Default): The percentage (0.0 to 1.0) of the loan balance lost after repossessing assets.
...
Ethical Bias Audit & Mitigation
Anti-discrimination laws state that approval distributions must satisfy the Disparate Impact ratio (DI≥0.80, the standard 80% rule).
...
risk managers can adjust the unprivileged group's (Female) approval boundary from the general threshold of PD≤0.35 to a customized threshold of PD≤0.41.
This compensates for credit history discrepancies, balancing group opportunity distributions and moving the disparate impact meter back into the green legally compliant zone.
